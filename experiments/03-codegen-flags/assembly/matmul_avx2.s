...
# options passed: -mavx2 -mfma -mtune=generic -march=x86-64 -O3 -fasynchronous-unwind-tables -fstack-protector-strong -fstack-clash-protection -fcf-protection
	.text
	.p2align 4
	.globl	matmul
...
# src/matmul.c:8: void matmul(float A[N][N], float B[N][N], float C[N][N]){
	movq	%rdi, %r11	# tmp104, A
	movq	%rdx, %r10	# tmp106, C
...
# src/matmul.c:13:                 sum += A[i][k] * B[k][j];
	vmovss	(%rdx), %xmm2	# MEM[(float *)_31], tmp108
...
# src/matmul.c:15:             C[i][j] = sum;
	vmovss	%xmm0, (%rsi)	# sum, MEM[(float *)_8]
# src/matmul.c:10:         for(size_t j = 0; j < N; j++){
	addq	$4, %rsi	#, ivtmp.18
