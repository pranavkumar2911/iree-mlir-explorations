...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  flow.executable private @abs_dispatch_0 {
    flow.executable.export public @abs_dispatch_0_elementwise workgroups() -> (index, index, index) {
      %c1 = arith.constant 1 : index
      flow.return %c1, %c1, %c1 : index, index, index
    }
    builtin.module {
      func.func @abs_dispatch_0_elementwise(%arg0: !iree_tensor_ext.dispatch.tensor<readonly:tensor<f32>>, %arg1: !iree_tensor_ext.dispatch.tensor<writeonly:tensor<f32>>) {
        %0 = iree_tensor_ext.dispatch.tensor.load %arg0, offsets = [], sizes = [], strides = [] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<f32>> -> tensor<f32>
        %1 = tensor.empty() : tensor<f32>
        %2 = linalg.generic {indexing_maps = [#map, #map], iterator_types = []} ins(%0 : tensor<f32>) outs(%1 : tensor<f32>) {
        ^bb0(%in: f32, %out: f32):
          %3 = math.absf %in : f32
          linalg.yield %3 : f32
        } -> tensor<f32>
  ...
  util.func public @abs(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "sync func @abs(%input0: tensor<f32>) -> (%output0: tensor<f32>)"}} {
    ...
    %1 = flow.dispatch @abs_dispatch_0::@abs_dispatch_0_elementwise(%0) : (tensor<f32>) -> tensor<f32>
