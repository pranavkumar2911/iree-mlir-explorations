...
module attributes {stream.affinity.default = #hal.device.affinity<@__device_0>} {
  ...
  util.func public @abs(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub, iree.reflection = {iree.abi.declaration = "sync func @abs(%input0: tensor<f32>) -> (%output0: tensor<f32>)"}} {
    ...
    %1 = iree_tensor_ext.compute_barrier.start %0 : tensor<f32> -> tensor<f32>
    %2 = linalg.generic {indexing_maps = [#map, #map], iterator_types = []} ins(%1 : tensor<f32>) outs(%1 : tensor<f32>) {
    ^bb0(%in: f32, %out: f32):
      %5 = math.absf %in : f32
      linalg.yield %5 : f32
    } -> tensor<f32>
    %3 = iree_tensor_ext.compute_barrier.end %2 : tensor<f32> -> tensor<f32>
    %4 = hal.tensor.export %3 "output0" : tensor<f32> -> !hal.buffer_view
