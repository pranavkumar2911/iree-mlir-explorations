...
The flow dialect is where IREE chops the model into separate dispatches, each of which will become its own compiled kernel later. MobileNetV2 has roughly 150-180 ops in the raw ONNX graph and IREE fused this down to 36 dispatches. That's a ~5x reduction in kernel count.
...
In dispatch 9, IREE goes further and fuses the residual connection and the output is added to a tensor from a previous block all within the same dispatch. This is the inverted-residual pattern that defines MobileNetV2.
...
Why this matters: downstream passes don't need special cases for conv vs matmul vs depthwise and they share the same lowering machinery driven by the indexing maps. For a hardware vendor backend, this is significant: teaching IREE about a new accelerator becomes "lower linalg.generic to my hardware," not "implement separate paths for every ML op."
...
- They inherit fusion, the host/device split, the runtime, ABI, command buffer model with all the upstream infrastructure
